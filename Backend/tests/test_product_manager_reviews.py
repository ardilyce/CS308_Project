...
def test_product_manager_can_approve_pending_comment_for_delivered_product():
    # GIVEN: comment exists for a delivered product, status = PENDING (flag False)
    ...
    product = ScrapedProduct.objects.create(
        name="Review Device",
        model="RV-1",
        serialnumber="SN-RV-1",
        description="Device for review approval test",
        stock=5,
        price=1200,
        warranty="1y",
        distributer="ACME",
        category="Test",
        url="https://example.com/products/review-device",
    )
    ...
    # WHEN: product manager approves comment
    res = client.patch(
        f"/api/reviews/{review.id}/flag/",
        {"flag": True},
        format="json",
    )

    # THEN: comment becomes visible to users
    assert res.status_code == status.HTTP_200_OK
    review.refresh_from_db()
    assert review.flag is True
    ...
    assert found.get("comment") == "Great product"
