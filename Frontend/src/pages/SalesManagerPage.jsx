...
export default function SalesManagerPage() {
  ...
  const [invoiceRange, setInvoiceRange] = useState({ start: "2025-02-01", end: "2025-02-28" });
  const [financeRange, setFinanceRange] = useState({ start: "2025-01-01", end: "2025-02-28" });
  ...
  const chartData = useMemo(() => {
    const buckets = {};
    filteredLedger.forEach((row) => {
      const label = new Date(row.date).toLocaleDateString("en-US", { month: "short" });
      const product = products.find((p) => p.id === row.productId);
      const unitCost = row.cost ?? product?.cost ?? (product?.price || 0) * 0.5;
  ...
  const markReceived = (id) => {
    setRefunds((prev) => prev.map((r) => (r.id === id ? { ...r, returned: true } : r)));
  };
  ...
  const handleRefund = (id, decision) => {
    ...
    if (decision === "approved") {
      setProducts((prev) =>
        prev.map((p) => (p.id === request.productId ? { ...p, stock: p.stock + 1 } : p))
      );
      ...
      <header className="sales-header">
        <div>
          ...
          <p className="subtitle">
            Set discounts, notify wish lists, export invoices, measure profit/loss, and finalize refunds with stock and payment updates.
          </p>
        </div>
      ...
      <section className="panel-grid">
            ...
            <table>
              ...
              <tbody>
                {products.map((p) => {
                  ...
                  const preview = Math.max(0, p.price * (1 - (Number(discountRate) || 0) / 100));
                  return (
                    <tr key={p.id}>
                      <td>
                        <input
                      ...
                      <td>{p.wishlist.length ? `${p.wishlist.length} users` : "—"}</td>
                      <td>
                        <div>{formatCurrency(finalPrice)}</div>
                        {p.appliedDiscount && <span className="pill">-{p.appliedDiscount}% live</span>}
                      </td>
        ...
        <div className="panel">
            ...
            <div className="date-filters">
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setInvoiceRange((prev) => ({ ...prev, start: e.target.value }))}
                />
              </label>
              <label>
                To
                <input
                  ...
                  onChange={(e) => setInvoiceRange((prev) => ({ ...prev, end: e.target.value }))}
                />
              </label>
            </div>
          </div>
          ...
          <div className="actions-row">
            <button className="btn-ghost" onClick={() => handleInvoiceAction("print")}>🖨️ Print</button>
            <button className="btn-primary" onClick={() => handleInvoiceAction("pdf")}>⬇️ Save as PDF</button>
          </div>
        </div>
      </section>
        ...
        <div className="panel">
            ...
            <div className="date-filters">
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setFinanceRange((prev) => ({ ...prev, start: e.target.value }))}
                />
              </label>
              <label>
                To
                <input
                  ...
                  onChange={(e) => setFinanceRange((prev) => ({ ...prev, end: e.target.value }))}
                />
              </label>
            </div>
          </div>
          ...
          <div className="stat-row">
            ...
            <div className="stat-card">
              ...
              <strong className={financeSummary.profit >= 0 ? "positive" : "negative"}>
            ...
            {chartData.map((c) => {
              ...
              return (
                <div key={c.label} className="chart-row">
                  ...
                  <div className={`chart-bar ${c.value >= 0 ? "positive" : "negative"}`} style={{ width }}>
                    {formatCurrency(c.value)}
                  </div>
                </div>
              );
            })}
            {chartData.length === 0 && <p className="muted">No sales or refunds in this range.</p>}
          </div>
        </div>
              ...
              <div key={req.id} className="refund-card">
                <div className="refund-top">
                  <div>
                    ...
                    <h4>{req.product}</h4>
                    <p className="subtext">Purchased {req.purchaseDate} • {req.campaign}</p>
                  </div>
                  <div className="refund-amount">{formatCurrency(req.purchasePrice)}</div>
                </div>
                <div className="refund-details">
                  <span className="pill">Refund to: {req.refundMethod === "credit_card" ? "Credit Card" : "Account Balance"}</span>
                  <span className="pill">Returned: {req.returned ? "Yes" : "Awaiting product"}</span>
                  <span className="pill">Honor purchase price even if campaign ended</span>
                </div>
                <div className="refund-actions">
                  {!req.returned && (
                    <button className="btn-ghost" onClick={() => markReceived(req.id)}>
                  ...
                  <button className="btn-ghost" onClick={() => handleRefund(req.id, "rejected")}>
                    Reject
                  </button>
                  {req.status && <span className={`pill status-${req.status}`}>{req.status}</span>}
                </div>
              </div>
            ))}
          </div>
            ...
            {refundLog.length === 0 ? (
              ...
              refundLog.map((item) => (
                <div key={item.id} className="note">
                  <span>{item.note}</span>
                  <span className="time">{item.method === "credit_card" ? "Card" : "Account"}</span>
                </div>
              ))
            )}
          </div>
